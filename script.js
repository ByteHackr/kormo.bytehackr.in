...
document.addEventListener('DOMContentLoaded', function() {
    ...
    if (document.getElementById('resumeContent')) {
        ...
        loadPhotoSize(); // Load saved photo size
...
function updatePreview() {
    ...
    setTimeout(function() {
        ...
        // Apply saved photo size
        if (photoSize !== 100) {
            var photo = document.querySelector('.resume-content .profile-photo');
            if (photo) {
                var baseSize = 100;
                var newSize = (baseSize * photoSize / 100);
                photo.style.width = newSize + 'px';
                photo.style.height = newSize + 'px';
            }
        }
    }, 100);
}
...
// Initialize drag and drop for preview sections
function initPreviewDragDrop() {
    var resumeContent = document.getElementById('resumeContent');
    if (!resumeContent) return;
    
    // Add drag handlers after preview updates
    var sections = resumeContent.querySelectorAll('.resume-section');
    
    sections.forEach(function(section, index) {
        section.setAttribute('draggable', 'true');
        section.setAttribute('data-preview-index', index);
        
        section.addEventListener('dragstart', handlePreviewDragStart);
        section.addEventListener('dragend', handlePreviewDragEnd);
        section.addEventListener('dragover', handlePreviewDragOver);
        section.addEventListener('drop', handlePreviewDrop);
        section.addEventListener('dragleave', handlePreviewDragLeave);
    });
}

var draggedPreviewSection = null;

function handlePreviewDragStart(e) {
    draggedPreviewSection = this;
...
function handlePreviewDrop(e) {
    ...
    if (draggedPreviewSection && this !== draggedPreviewSection) {
        // Get the section names from the preview
        var draggedTitle = draggedPreviewSection.querySelector('h2');
        var targetTitle = this.querySelector('h2');
        
        if (draggedTitle && targetTitle) {
            var draggedName = getSectionNameFromTitle(draggedTitle.textContent);
            var targetName = getSectionNameFromTitle(targetTitle.textContent);
            
            // Reorder in the form
            reorderFormSections(draggedName, targetName);
        }
    }
}

function getSectionNameFromTitle(title) {
    ...
    if (titleLower.includes('project') || titleLower.includes('accomplishment')) return 'projects';
    if (titleLower.includes('certification')) return 'certifications';
    if (titleLower.includes('skill')) return 'skills';
    return 'custom';
}

function reorderFormSections(draggedName, targetName) {
    var container = document.getElementById('movableSections');
    if (!container) return;
    
    var draggedSection = container.querySelector('[data-section="' + draggedName + '"]');
    var targetSection = container.querySelector('[data-section="' + targetName + '"]');
...
// ============================================
// INTERACTIVE PREVIEW - PHOTO RESIZE
// ============================================

var photoSize = 100; // Default size percentage

function initPhotoResize() {
    // This will be called after preview updates
    var resumeContent = document.getElementById('resumeContent');
    if (!resumeContent) return;
    
    var photo = resumeContent.querySelector('.profile-photo');
    if (!photo) return;
    
    // Add click handler for size popup
    photo.addEventListener('click', function(e) {
        e.stopPropagation();
        togglePhotoSizePopup(photo);
    });
}

function togglePhotoSizePopup(photoElement) {
    // Remove any existing popup
    var existingPopup = document.querySelector('.photo-size-popup');
    if (existingPopup) {
    ...
    popup.className = 'photo-size-popup visible';
    popup.innerHTML = `
        <label>Photo Size: <span id="photoSizeValue">${photoSize}%</span></label>
        <input type="range" min="50" max="150" value="${photoSize}" 
               oninput="updatePhotoSize(this.value)">
        <div style="display: flex; gap: 0.5rem; margin-top: 0.5rem;">
            <button onclick="updatePhotoSize(80)" style="flex:1; padding: 0.25rem; font-size: 0.75rem; cursor: pointer;">S</button>
            <button onclick="updatePhotoSize(100)" style="flex:1; padding: 0.25rem; font-size: 0.75rem; cursor: pointer;">M</button>
            <button onclick="updatePhotoSize(120)" style="flex:1; padding: 0.25rem; font-size: 0.75rem; cursor: pointer;">L</button>
        </div>
    `;
    
    photoElement.style.position = 'relative';
    photoElement.appendChild(popup);
    
    // Close popup when clicking outside
    document.addEventListener('click', function closePopup(e) {
        if (!popup.contains(e.target) && e.target !== photoElement) {
            popup.remove();
...
function updatePhotoSize(size) {
    ...
    // Update display
    var sizeValue = document.getElementById('photoSizeValue');
    if (sizeValue) sizeValue.textContent = photoSize + '%';
    
    // Update slider
    var slider = document.querySelector('.photo-size-popup input[type="range"]');
    if (slider) slider.value = photoSize;
    
    // Apply size to photo
    var photo = document.querySelector('.resume-content .profile-photo');
    if (photo) {
        var baseSize = 100; // Base size in pixels
        var newSize = (baseSize * photoSize / 100);
        photo.style.width = newSize + 'px';
        photo.style.height = newSize + 'px';
    }
    ...
    localStorage.setItem('kormoNamaPhotoSize', photoSize);
}

function loadPhotoSize() {
    var saved = localStorage.getItem('kormoNamaPhotoSize');
    if (saved) {
        photoSize = parseInt(saved);
    }
}

// ============================================
// SECTION REORDERING (Form)
// ============================================
